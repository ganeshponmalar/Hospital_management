...
const PurchasesTab = () => {
    ...
    const fetchSelectableData = async () => {
        try {
            ...
            const [supRes, medRes, purRes] = await Promise.all([
                ...
                axios.get('http://localhost:5000/api/pharmacy/purchases', config)
            ]);
    ...
    const handlePurchase = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post('http://localhost:5000/api/pharmacy/purchases', 
                { supplier_id: selectedSupplier, purchase_date: purchaseDate, items },
                { headers: { Authorization: `Bearer ${token}` }}
            );
            setSelectedSupplier('');
            setItems([]);
